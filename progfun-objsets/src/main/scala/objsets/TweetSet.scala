...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    if (empty) Nil else new Cons(this.mostRetweeted, this.remove(mostRetweeted).descendingByRetweet) 
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted = {
    def maxTweet(tw1:Tweet,tw2:Tweet) = if (tw1.retweets > tw2.retweets) tw1 else tw2
    
    if (left.empty && right.empty) elem
    else if (left.empty) maxTweet(elem,right.mostRetweeted)
    else if (right.empty) maxTweet(left.mostRetweeted, elem)
